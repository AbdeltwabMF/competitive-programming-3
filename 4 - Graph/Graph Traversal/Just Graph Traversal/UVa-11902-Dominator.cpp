...
int Head[109], Next[10109], To[10109], ne, n, u;
char vis[109], reachable[109], ans[109][109];

void DFS(int node, int tar = -1)
{
    if(node == tar) return;
...
void Solve()
{
    ...
    DFS(0);
    for(int i = 0; i < n; ++i) reachable[i] = vis[i];
    ...
    for(int i = 0; i < n; ++i)
    {
        ...
        for(int j = 0; j < n; ++j)
        {
            if(reachable[j] && !vis[j]) ans[i][j] = 'Y';
            else ans[i][j] = 'N';
        }
    }
