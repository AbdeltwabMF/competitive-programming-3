...
int Head[109], Next[209], in[109], To[209], ne, n, m;
map <string, int> inx;
string arr[109], u, v;
...
void _clear()
{
    ...
    inx.clear(); ready = priority_queue <int> (); t_sort.clear();
    ne = 0;
}
...
void Solve()
{
    ...
    while(ready.size())
    {
        tmp = -ready.top(); ready.pop();
        t_sort.push_back(tmp);

        for(int i = Head[tmp]; i; i = Next[i]) if(--in[To[i]] == 0)
            ready.push(-To[i]);
    }

    for(int i = 0; i < sz(t_sort); ++i)
        cout << arr[t_sort[i]] << " ."[i == sz(t_sort) - 1];
