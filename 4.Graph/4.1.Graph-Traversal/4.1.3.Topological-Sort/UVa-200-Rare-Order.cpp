...
int Head[30], Next[M << 1], To[M << 1], ne, st = oo, en = 0;
vector <int> t_sort;
bool vis[30];
string s;
vector <string> v;
...
void Solve()
{
    ...
    {
        if(v[i][0] != v[i + 1][0])
            addEdge((int)v[i][0] - 64, (int)v[i + 1][0] - 64);
        else  if(sz(v[i]) == sz(v[i + 1]))
        {
            for(int j = 0; j < sz(v[i]); ++j)
                if(v[i][j] != v[i + 1][j])
                {
                    addEdge((int)v[i][j] - 64, (int)v[i + 1][j] - 64);
                    break;
                }
        }
    }

    for(int j = st; j <= en; ++j) if(!vis[j])
        DFS(j);
