...
#define  _get(n)             find_by_order((n))
#define  _indexOf(n)         order_of_key((n))
...
{ os << "["; foreach(it, _ist) { if(it != _ist.begin() ) os << ", "; os << "(" << _ist._indexOf(*it) << ", " << *it << ")"; } return os << "]"; }
...
int Head[1 << 5], Next[1 << 6], To[1 << 6], ne;
vector <int> arr;
char u, v, ch, sn;
bool vis[1 << 5], isValidOrder;
string line;
stringstream ss;

bool valid(int node)
{
    for(int i = Head[node]; i; i = Next[i]) if(vis[To[i]]) return false;
    return true;
}

void back_track(string ts)
{
    if(sz(ts) == sz(arr))
    {
        isValidOrder = true;
    ...
    for(int i = 0; i < sz(arr); ++i) if(!vis[arr[i]] && valid(arr[i]))
    {
        vis[arr[i]] = true;
        back_track(ts + char(arr[i] + 64));
        vis[arr[i]] = false;
    }
}
